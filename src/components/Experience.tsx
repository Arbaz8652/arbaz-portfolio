        ...
        <div className="relative">
          ...
          {experiences.map((exp, i) => (
            ...
            >
              ...
              {/* Content */}
              <div className={`lg:w-1/2 pl-8 lg:pl-0 ${i % 2 === 0 ? 'lg:pr-12 lg:text-right' : 'lg:pl-12'}`}>
                <div className="p-6 card-gradient border border-border rounded-xl hover:border-primary/50 transition-all duration-300">
                  ...
                  <div className={`flex items-center gap-4 mb-4 text-sm text-muted-foreground ${i % 2 === 0 ? 'lg:justify-end' : ''}`}>
                  ...
                  <ul className={`space-y-2 ${i % 2 === 0 ? 'lg:text-right' : ''}`}>
                    {exp.achievements.map((achievement, j) => (
                      <li key={j} className="text-sm text-muted-foreground flex items-start gap-2">
                        <span className={`text-primary mt-1.5 ${i % 2 === 0 ? 'lg:order-2' : ''}`}>•</span>
